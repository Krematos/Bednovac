...
public class ApiClient {
     ...
     *
     * @param caseName Název bedny (např. "Kilowatt Case"). Musí přesně odpovídat formátu API.
     * @return CompletableFuture, který obsahuje instanci Case s aktualizovanou cenou, nebo null v případě chyby.
     */
    ...
    public CompletableFuture<Case> fetchCasePrice(String caseName) {
        ...
        try{
            ...
            String url = String.format("https://steamcommunity.com/market/priceoverview/?appid=%s&currency=%s&market_hash_name=%s", "730", "6",
                    encodedCaseName); // URL pro získání ceny bedny z API
                    ...
                    .thenApply(response -> {
                        try{
                            ...
                            if(lowestPriceStr == null || lowestPriceStr.isEmpty()) {
                                LOGGER.log(Level.WARNING, "Lowest price not found for case: " + caseName + ", response: " + response);
                                return null; // Pokud lowest_price není v odpovědi, vrátíme null
                            }
                            // Odstranění měny a převod na číslo
                            String normalized = lowestPriceStr.replaceAll("[^0-9,\\.]", "").replace(",", "."); // Nahrazení čárky tečkou pro desetinnou část
                            ...
                            return null; // Pokud dojde k chybě při parsování, vrátíme null
                        }
                    })
                    .exceptionally(e -> {
                        LOGGER.log(Level.WARNING, "Failed to fetch case price for: " + caseName, e);
                        return null; // Pokud dojde k chybě při volání API, vrátíme null
                    });
        } catch (Exception e){
            LOGGER.log(Level.WARNING, "Failed to encode case name: " + caseName, e);
            return CompletableFuture.completedFuture(null); // Pokud dojde k chybě při kódování, vrátíme null
        }
     ...
     * Tato metoda předpokládá API jako ExchangeRate-API.com s USD jako základní měnou.
     *
     * @return CompletableFuture, který obsahuje instanci ExchangeRate s aktuálními kurzy, nebo null v případě chyby.
     */

    public CompletableFuture<Exchange> fetchExchangeRates() {
        String url = "https://api.frankfurter.app/latest?base=USD"; // URL pro získání směnných kurzů s USD jako základní měnou
                ...
                .thenApply(responseBody -> {
                    ...
                    try {
                        ...
                        if (rootNode.has("error-type")) {
                            ...
                            return null; // Pokud API vrátí chybu, vrátíme null
                        }
