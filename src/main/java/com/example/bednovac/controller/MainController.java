...
/**
 * Controller pro hlavní okno aplikace, který zpracovává události a interakce s uživatelským rozhraním.
 * Tato třída implementuje Initializable pro inicializaci komponent po načtení FXML souboru.
 */

public class MainController implements Initializable { // Implementujte Initializable pro inicializaci
    ...
    /**
     * Inicializace controlleru. Tato metoda bude volána po načtení FXML souboru.
     */
    @Override
    public void initialize(URL url, ResourceBundle resources) {
        LOGGER.info("Initializing MainController...");
        currencyChoiceBox.getItems().addAll(Currency.values());
        currencyChoiceBox.getSelectionModel().select(Currency.USD); // Nastaví výchozí měnu na CZK
    ...
    /**
     * Metoda pro zpracování události kliknutí na tlačítko exchangeButton.
     */
    @FXML
    public void handleExchangeButtonClick() {
        exchangeButton.setDisable(true); // Zabráníme opakovanému kliknutí
        LOGGER.info("Exchange button clicked.");
        // Získání částky z textového pole
        String amountText = amountTextField.getText().replace(",", ".").replace(" ",""); // Nahraď čárku tečkou pro parsování
        if (amountText.isEmpty()) {
            Platform.runLater(() -> { // Aktualizace UI vždy na FX threadu
        ...
        LOGGER.info("Starting calculation for amount: " + amount + " " + selectedCurrency.name() + " for case: " + selectedCase);

        // 2. Asynchronní získání cen a výpočet
        ...
        // Kombinace výsledků obou asynchronních volání
        CompletableFuture.allOf(casePriceFuture, exchangeRateFuture)
                .thenAccept(v -> { // thenAccept je vhodné, když nechceme vracet další CompletableFuture
                    try {
                        ...
                        double keyPriceUsd = getPriceService().getKeyPrice(); // Získá cenu klíče

                        // VÝPOČET:
                        // 1. Převeď uživatelskou částku na USD
                        // Předpoklad: exchangeRate je "Kolik CZK (nebo jiné měny) je za 1 USD" (např. 23.5 pro CZK)
                        // Proto DĚLÍME zadanou částku kurzem, abychom dostali USD
                        double userAmountUsd = amount / exchangeRate;

                        // 2. Vypočítej celkovou cenu jednoho balíčku (bedna + klíč) v USD
                        ...
                        // 3. Vypočítej, kolik celých balíčků si uživatel může koupit
                        int numberOfBundles = (int) (userAmountUsd / totalBundleCostUsd);

                        // 4. Vypočítej zbytek peněz v USD
                        double remainingUsd = userAmountUsd % totalBundleCostUsd;

                        // 5. Převeď zbytek peněz zpět na původní měnu
                        double remainingInSelectedCurrency = remainingUsd * exchangeRate; // NÁSOBÍME pro zpětný převod

                        // 6. Aktualizace UI (VŽDY na JavaFX aplikačním threadu!)
                        Platform.runLater(() -> {
                            casesLabel.setText(String.format("Bedny: %d", numberOfBundles));
                            keysLabel.setText(String.format("Klíče: %d", numberOfBundles)); // Stejný počet klíčů jako beden
                ...
                .exceptionally(e -> { // Zachycení chyb z CompletableFuture.allOf
                    ...
                    return null; // Důležité pro exceptionally
                });
    }


    /**
     * Metoda pro zpracování události kliknutí na tlačítko resetButton.
     */
    @FXML
    ...
    // getters and setters for FXML components
